...
Next, we will import our own data. This data has been included in this package, so you can import it using the `rio` library. This code has been provided to help you see how to import data included in the exercises. Give your new dataset a name by replacing `_____` with a variable name. 

This dataset includes data from a study our research team did on typing, looking at how pleasant people rated words based on the way a word is typed on a QWERTY keyboard, which hand the participant wrote with, and more. 

```{r data-wrangle, exercise=TRUE}
library(rio)
______ <- import("introR.csv")
```

```{r data-wrangle-solution}
library(rio)
DF <- import("introR.csv")
```
